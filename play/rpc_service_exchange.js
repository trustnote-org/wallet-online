...
function initRPC() {
	...
	server.expose('exchangetofile1', function(args, opt, cb) {
		console.log('exchangetofile '+JSON.stringify(args));

		var from_address =  args[0];
		var to_address =  args[1];
		var amount =  args[2];
		if (amount && to_address && from_address) {
			if (validationUtils.isValidAddress(from_address) && validationUtils.isValidAddress(to_address)){
				function writeUnits(objUnit){
					fs.writeFile(UNSIGNUNITS_FILENAME, JSON.stringify(objUnit, null, '\t'), 'utf8', function(err){
						...
						cb(err, err ? undefined : "Exchange to file succeed!");
					});
				}
				function onError(err){
					throw Error(err);
				...
				var arrOutputs = [
					...
					{address: to_address, amount: amount}  // the receiver
				];
	...
	server.expose('exchangetochain1', function(args, opt, cb) {
		console.log('exchangetochain'+JSON.stringify(args));
		...
		fs.readFile(SIGNEDUNITS_FILENAME, 'utf8', function(err, data){
			if (err){
				console.log('Failed to read signed units.');
				cb(undefined, "Failed to read signed units.");
			}
			else{
