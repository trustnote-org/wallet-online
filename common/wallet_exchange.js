...
var composer = require('./composer_exchange.js');
...
function sendPaymentFromWalletExchange(
	asset, wallet, to_address, amount, change_address, arrSigningDeviceAddresses, recipient_device_address, signWithLocalPrivateKey, handleResult)
{
	sendMultiPaymentExchange({
		asset: asset,
		wallet: wallet,
		to_address: to_address,
		amount: amount,
...
function sendMultiPaymentExchange(opts, handleResult)
{
	var asset = opts.asset;
	if (asset === 'base')
		asset = null;
	...
	readFundedAndSigningAddresses(
		...
		function(arrFundedAddresses, arrBaseFundedAddresses, arrAllSigningAddresses){
			...
			function writeUnits(objUnit){
				//get max address , used by signer to catch up address index
				function GetMaxAddressIndex(objUnit, ischange, cb) {
					db.query("SELECT max(address_index) as max FROM my_addresses WHERE is_change = ?", [ischange], function(outputsRows) {
						if (outputsRows.length === 0)
							cb(0);
						cb(outputsRows[0]["max"]);
					});
				}
				var objUnit = {"objUnit" : objUnit};
				GetMaxAddressIndex(objUnit, 0, function(max){
					objUnit.max_main = max;
					GetMaxAddressIndex(objUnit, 1, function(max){
						objUnit.max_change = max;
						fs.writeFile(UNSIGNUNITS_FILENAME, JSON.stringify(objUnit, null, '\t'), 'utf8', function(err){
							if (err)
								handleResult("failed to write units file");
							handleResult(null, JSON.stringify(objUnit, null, '\t'));
						});
					});
				});
			}
			...
			var params = {
				...
				callbacks: {
					...
					// for asset payments, 2nd argument is array of chains of private elements
					// for base asset, 2nd argument is assocPrivatePayloads which is null
					ifOk: function(objJoint, arrChainsOfRecipientPrivateElements, unlock_callback){
						// network.broadcastJoint(objJoint);
						// if (!arrChainsOfRecipientPrivateElements && recipient_device_address) // send notification about public payment
						// 	walletGeneral.sendPaymentNotification(recipient_device_address, objJoint.unit.unit);
						// handleResult(null, objJoint.unit.unit);
						//modi exchange : Don't broadcast,just write to file
						unlock_callback();
						writeUnits(objJoint);
					}
				}
...
exports.sendPaymentFromWalletExchange = sendPaymentFromWalletExchange;
